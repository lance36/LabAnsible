...
(in new terminal)  
    ...
    cd LabAnsible/LabAnsibleKubernetesWithTrident_NAS_iSCSI
...
# Copy the Trident backend file in /etc/netappdvp
    cp ~/LabAnsible/LabAnsibleKubernetesWithTrident_NAS_iSCSI/0-Trident-config/config-ontap-nas.json /etc/netappdvp/
